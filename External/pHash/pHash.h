 ...
 */
__declspec(dllexport) int __cdecl ph_compare_images(const char *file1, const char *file2,double &pcc, double sigma = 3.5, double gamma=1.0, int N=180,double threshold=0.90);
 ...
 */
#ifdef HAVE_IMAGE_HASH
__declspec(dllexport) int __cdecl ph_hamming_distance(const ulong64 hash1,const ulong64 hash2);
 ...
 **/
__declspec(dllexport) double __cdecl ph_hammingdistance2(uint8_t *hashA, int lenA, uint8_t *hashB, int lenB);
