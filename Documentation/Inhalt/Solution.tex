...
\label{sec:solution}
Für das Verständnis der sowohl grundlegenden Funktionsweisen der verschiedenen Algorithmen als auch die Wahl des
...
einzelnen Farbkanäle (rot, grün und blau) definiert. Ein Graustufenbild besitzt dahingegen nur einen Farbkanal.
\\
...
welches strukturell in hochfrequente als auch in niederfrequente Bereiche unterteilt ist. Gleichmäßige Unterschiede innerhalb
der Bildstruktur werden durch die niederfrequenten Anteile abgebildet, wohingegen hochfrequente Anteile[SYN] mit ungleichmäßigen
sowie plötzlichen strukturellen Änderungen zum Detail des Gesamtbilds beitragen.
...
Diese Kategorie beinhaltet 9 verschiedenfarbige Bilder mit den nachstehenden Spezifikationen. Der Farbbereich deckt dabei
sowohl hell als auch dunkle Farben ab:\\
TO DO!

\boldtext{Elementare Formen}\\
Die Kategorie ist in drei unterschiedliche Gruppen[SYN] mit je 54 Bildern eingeteilt, wobei die Bildmotive abhängig von der
Gruppierung variieren. Die nachfolgenden Abbildung~\ref{fig:appendix_b_cc} schafft einen Überblick über die verwendeten Muster
...
\end{figure}
\noindent
Die drei Gruppen umfassen neben den Originalbildern ebenfalls durch zum Beispiel Filter verändertes Bildmaterial. Folgende
Änderungen sind auf die originalen Bilder angewendet worden: \textit{Skalierungsänderungen, Rotationen, Gammaanpassungen und Noise}.
...
ist dem Anhang~\ref{sec:appendix_b} (\textit{Dokumentation der verwendeten Daten}) zu entnehmen.

\boldtext{Komplexe Bilder}\\
Die komplexen Bilder sind ähnlich zu der Kategorie \textit{Elementare Formen}. Diese beinhaltet nur eine Gruppe mit 21 verschiedenen
ähnlichen sowie nicht ähnlichen Fotos, die folgenden Spezifikationen aufweisen:\\
TO DO!\\
Die Änderungen mittels von zum Beispiel Filtern entsprechen den Anpassungen aus der Kategorie \textit{Elementare Formen}.
...
der anschließende Vergleich der errechneten Hashes kann mit der nachfolgenden Tabelle~\ref{tab:solution_algorithms}
...
\end{table}	
\noindent
Gemeinsamkeiten in dem internen Ablauf der vier unterschiedlichen Ansätze sind in der Umwandlung des Farbbilds zu einen Graustufenbild
sowie in der Verminderung[SYN] der Auflösung des Bildes gegeben. Eine Ausnahme macht der Algorithmus \textit{RADISH}, welcher keine
Verkleinerung des Bildmaterials vorsieht.
\\
Beide Vorbearbeitungsschritte dienen lediglich einer Verbesserung des Berechnungsaufwands. Durch die Umwandlung des Bilds in den
Graustufenbereich und somit der Reduzierung der drei Farbkanäle auf einen Farbkanal, gehen keine für die Algorithmen erforderlichen
Strukturinformationen verloren. Gleiches besitzt Gültigkeit für die Reduzierung[SYN] der Gesamtbildinformationen: die strukturellen 
Informationen bleiben bestehen.
...
durchgeführt, die als Ergebnis für die Extraktion von Merkmalen dient. Der resultierende Hash wird mit Hilfe der bestimmten[SYN]
Spektralkomponenten des Merkmalbildes[SYN] errechnet.
\\
...
Diesem Verfahren werden als weitere Vorbearbeitungsschritte ein Blurring sowie einen Histogramm basierten Ausgleich der Bilddaten
vorangestellt. Die Bestimmung der Bild abhängigen Merkmale und dem drauf folgenden Hashwert geschieht mittels einem
\textit{Laplacian of Gaussian}--Kernel für die Kantenerkennung.
...
\\
Das Verfahren bietet in der Theorie ebenfalls zwei rotationsvorbeugende Vorgehensweisen, welche nicht in der Bibliothek \textit{pHash}
