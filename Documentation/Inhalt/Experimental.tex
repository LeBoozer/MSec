...
Im Experiment wird ein Quervergleich aller neun einfarbigen Bilder der Kategorie \textit{Einfache Farben} mit allen vier Verfahren durchgeführt.
...
Für die Sicherstellung, dass die ermittelten Zeiten unverfälscht gemessen werden und somit die Ansprüche an die Wiederholbarkeit
ausreichend sind, werden keine Optimierungsschritte integriert (wie zum Beispiel das einmalige Laden der Bildinformationen und die
anschließende wiederholte Verwendung der Daten).
\\
...
das ein Maß der Ähnlichkeit von zwei unterschiedlichen Signalen beschreibt (vgl.~\cite{MAR95}). In dem vorliegenden Experiment werden dahingegen
...
\textit{Komplexe Bilder} ausgeführt. Für eine verbesserte Möglichkeit zur Protokollierung der zeitlichen Entwicklung hinsichtlich
