...
und trägt somit zu der \textit{FRR} bei, während die \textit{Kreuzkorrelation} bei gleichen Hashwerten stets in einem
...
Testfalls die \textit{FAR} während des Vergleichs von nicht ähnlichen Bildmaterials untersuchte.
...
\\[1em]
Ähnlich wie bei den einfachen Farben zuvor, beeinflusst der strukturell einfach aufgebaute Inhalt der Bilder auch hier die
im Abschnitt~\ref{sec:experimental_origvsmod_brushes} protokollierten Ergebnisse negativ. Bereits kleinere Änderungen an dem originalen
...
\begin{figure}[H]
	...
	\caption{Wavelet Schritt für Schritt (Eigene Darstellung)}
...
\end{figure}
\noindent
Voraussetzung für die reduzierte Einflussnahme ist, dass die angewandten Veränderungen eine bestimmte Magnitude nicht
...
guten Fehlerverteilung für die einzelnen Algorithmen. Die gemessenen Fehlerraten lagen dabei auch bei einem Threshold von 70\%
beständig weit unter der 10\%--Grenze.
\\
Der Fehlerbestand bei einer Stufe von 90\% als Threshold besteht dabei vorwiegend aus ähnlichen Bildern. Erst mit der Verringerung des
...
analysierte, untersuchte der zweite Test die prozentuale Effizienzsteigerung der vier Algorithmen gegenüber eines nativen Ansatzes.
...
Möglichkeit, den Hashwert für das Bild im Vorfeld zu berechnen und beispielsweise in einer Datenbank zu sichern. Daraus folgt
...
angewiesen, sodass zum Beispiel eine Zwischenspeicherung der im Vorfeld bekannten Teilergebnisse nicht möglich ist.
\\[1em]
