...
\label{sec:results}
Nachfolgend werden die Resultate sowie Beobachtungen der durchgeführten Experimente analysiert und gegebene Korrelationen
...
bei strukturell ähnlichen Bildmaterials.
\\[1em]
Wie bei den einfachen Farben zuvor, beeinflusst der strukturell einfach aufgebaute Inhalt der Bilder auch hier negativ die
...
binären Repräsentation eine große Herausforderung darstellt. Der Erkenntnisgewinn lehnt sich an die Ergebnisse aus~\cite{ZAU10} an.
...
die entstehende Fehlerrate der verschiedenen Algorithmen während der Bestimmung von den Ähnlichkeitsmaßen.
\\
...
\\[1em]
Ähnlich zueinander liegende Intensitätsverteilungen, bei zwei nicht gleichartiger Bilder, resultieren bei dem \textit{RADISH}--Ansatz
in einer ähnlichen strukturellen Charakteristiken innerhalb der Ergebnisse der \textit{Radon--Projektion}, wodurch schlussendlich
ähnliche[SYN] Hashwerte erzielt werden (in Bezug auf den verwendeten Threshold).
\\
Die folgenden Abbildung~\ref{fig:results_cc_radish} ist ein Beispiel für das Phänomen. 
\begin{figure}[H]
	\centering
